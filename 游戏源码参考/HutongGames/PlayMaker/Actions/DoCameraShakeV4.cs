...
namespace HutongGames.PlayMaker.Actions
{
	public class DoCameraShakeV4 : FsmStateAction
	{
		...
		private void DoShake()
		{
			...
			CameraManagerReference cameraManagerReference = Camera.Value as CameraManagerReference;
			CameraShakeProfile cameraShakeProfile = Profile.Value as CameraShakeProfile;
			bool value = vibrate.Value;
			if (cameraManagerReference != null && (bool)cameraShakeProfile)
			{
				GameObject safe = Target.GetSafe(this);
				if (!MaxCameraDistance.IsNone && (bool)safe)
				{
					cameraManagerReference.DoShakeInRange(cameraShakeProfile, base.Owner, MaxCameraDistance.Value, safe.transform.position, DoFreeze.Value, value);
				}
				else
				{
					cameraManagerReference.DoShake(cameraShakeProfile, base.Owner, DoFreeze.Value, value);
				}
			}
			didShake = true;
			Finish();
