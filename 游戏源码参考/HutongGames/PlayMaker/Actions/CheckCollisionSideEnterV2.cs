...
namespace HutongGames.PlayMaker.Actions
{
	...
	public sealed class CheckCollisionSideEnterV2 : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			checkVelocity = !velocity2D.IsNone;
			if (checkVelocity)
			{
				velocityVector = velocity2D.Value;
			}
		...
		private void CheckTouching(int layerMask)
		{
			...
			leftHit.Value = false;
			if (checkUp && (!checkVelocity || velocityVector.y >= -0.001f))
			{
				topRays.Clear();
			...
			if (checkRight && (!checkVelocity || velocityVector.x >= -0.001f))
			{
				rightRays.Clear();
				rightRays.Add(max);
			...
			if (checkDown && (!checkVelocity || velocityVector.y <= 0.001f))
			{
				bottomRays.Clear();
			...
			if (!checkLeft || (checkVelocity && !(velocityVector.x <= 0.001f)))
			{
				return;
			}
			leftRays.Clear();
