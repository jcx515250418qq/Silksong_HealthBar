...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class DistanceFlyV3 : RigidBody2dActionBase
	{
		...
		private void DoBuzz()
		{
			...
			{
				if (stayLeft.Value && !stayRight.Value && self.Value.transform.position.x > target.Value.transform.position.x + 1f)
				{
			...
			if (targetsHeight)
			{
				if (!maxHeight.IsNone && maxHeightPrecedence.Value && maxHeight.Value != 0f && self.Value.transform.position.y > maxHeight.Value)
				{
					linearVelocity.y -= acceleration.Value;
				}
				else if (self.Value.transform.position.y < target.Value.transform.position.y + height.Value || self.Value.transform.position.y < target.Value.transform.position.y + minHeightAboveHero.Value)
				{
					linearVelocity.y += acceleration.Value;
				}
				else if (!maxHeight.IsNone && maxHeight.Value != 0f && self.Value.transform.position.y > maxHeight.Value)
				{
					linearVelocity.y -= acceleration.Value;
				}
				else if (self.Value.transform.position.y > target.Value.transform.position.y + height.Value)
				{
					linearVelocity.y -= acceleration.Value;
				}
			}
