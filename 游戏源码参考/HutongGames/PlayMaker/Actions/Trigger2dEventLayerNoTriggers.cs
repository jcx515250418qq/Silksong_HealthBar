...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class Trigger2dEventLayerNoTriggers : FsmStateAction
	{
		...
		{
			if (trigger == PlayMakerUnity2d.Trigger2DType.OnTriggerEnter2D && !collisionInfo.isTrigger && (collisionInfo.gameObject.tag == collideTag.Value || collideTag.IsNone || string.IsNullOrEmpty(collideTag.Value)) && (collisionInfo.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
		...
		{
			if (trigger == PlayMakerUnity2d.Trigger2DType.OnTriggerStay2D && !collisionInfo.isTrigger && (collisionInfo.gameObject.tag == collideTag.Value || collideTag.IsNone || string.IsNullOrEmpty(collideTag.Value)) && (collisionInfo.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
				StoreCollisionInfo(collisionInfo);
				base.Fsm.Event(sendEvent);
			}
		...
		{
			if (trigger == PlayMakerUnity2d.Trigger2DType.OnTriggerExit2D && !collisionInfo.isTrigger && (collisionInfo.gameObject.tag == collideTag.Value || collideTag.IsNone || string.IsNullOrEmpty(collideTag.Value)) && (collisionInfo.gameObject.layer == collideLayer.Value || collideLayer.IsNone))
			{
				StoreCollisionInfo(collisionInfo);
				base.Fsm.Event(sendEvent);
			}
