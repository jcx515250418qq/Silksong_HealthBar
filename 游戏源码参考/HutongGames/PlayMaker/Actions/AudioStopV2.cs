	...
	public class AudioStopV2 : ComponentAction<AudioSource>
	{
		...
		public override void OnEnter()
		{
			if (UpdateCache(base.Fsm.GetOwnerDefaultTarget(gameObject)))
			{
				...
				{
					fadeRoutine = StartCoroutine(VolumeFade(base.audio, 0f, fadeTime.Value));
				}
			}
			Finish();
		}
		...
		private IEnumerator VolumeFade(AudioSource audioSource, float endVolume, float fadeDuration)
		{
			...
			{
				if (!hasStopped && (originalClip == null || base.audio.clip == originalClip))
				{
					hasStopped = true;
					audioSource.Stop();
				}
		...
		public override void OnExit()
		{
			if (fadeRoutine != null)
			{
				...
				if (!cancelOnEarlyExit && base.audio != null && !hasStopped && (originalClip == null || base.audio.clip == originalClip))
				{
					hasStopped = true;
					base.audio.Stop();
				}
