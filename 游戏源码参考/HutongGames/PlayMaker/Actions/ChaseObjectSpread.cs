...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class ChaseObjectSpread : RigidBody2dActionBase
	{
		...
		private void DoBuzz()
		{
			...
			if (targetSpreadX.Value > 0f || targetSpreadY.Value > 0f)
			{
				if (timer >= spreadResetTime)
				{
					spreadX = Random.Range(0f - targetSpreadX.Value, targetSpreadX.Value);
					spreadY = Random.Range(0f - targetSpreadY.Value, targetSpreadY.Value);
					timer = 0f;
