	...
	public class RayCast2dIgnoreTag : FsmStateAction
	{
		...
		private void DoRaycast()
		{
			...
			else
			{
				...
				closestHit = Helper.Raycast2D(value, normalized, num, ActionHelpers.LayerArrayToLayerMask(layerMask, invertMask.Value), num2, num3);
			}
			if (closestHit.collider != null && closestHit.collider.gameObject.tag != ignoreTag.Value)
			{
			...
			if (debug.Value)
			{
				...
				Vector3 end = vector2 + vector3 * num4;
